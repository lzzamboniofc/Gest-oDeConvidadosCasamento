...
const sample = {
  weddings: [{ id: "demo-wedding", name: "Liliane & Igor", slug: "liliane-igor", event_date: "2027-04-03", invite_base_url: "../ConviteCasamento-Nivel3-Integrado/index.html" }],
  invitations: [
...
const buildLink = (invite, wedding) => {
  ...
  try { return new URL(`${sep}convite=${encodeURIComponent(invite.token)}`, base).href; } catch { return `${base}${sep}convite=${encodeURIComponent(invite.token)}`; }
};
...
function bindActions(){
  ...
  $("#logoutButton").addEventListener("click",async()=>{await backend.logout();if (CONFIG.mode === "supabase") location.replace(CONFIG.loginPage || "login.html");else location.reload();});
}
...
async function start(){
  ...
  if (CONFIG.mode === "supabase" && !state.user) { location.replace(CONFIG.loginPage || "login.html"); return; }
  ...
  if (CONFIG.mode === "supabase") supabase.auth.onAuthStateChange((event) => { if (event === "SIGNED_OUT") location.replace(CONFIG.loginPage || "login.html"); });
}
